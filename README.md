...
- [AetherCore-Dev/ag402-mcp](https://github.com/AetherCore-Dev/ag402) [glama](https://glama.ai/mcp/servers/AetherCore-Dev/ag402-mcp) 🐍 ☁️ - Payment layer for AI agents using Coinbase x402 protocol. Wrap any API or MCP server with a USDC paywall in one command, or let agents auto-pay for paid APIs. Solana USDC, ~0.5s settlement, non-custodial, 648+ tests. Works with Claude Code, Cursor, OpenClaw, LangChain, AutoGen out of the box.
- [AetherCore-Dev/token-rugcheck](https://github.com/AetherCore-Dev/token-rugcheck) [glama](https://glama.ai/mcp/servers/AetherCore-Dev/token-rugcheck) 🐍 ☁️ - Solana token safety audit for AI agents. Three-layer risk analysis (machine verdict + LLM analysis + raw on-chain evidence) from RugCheck.xyz, DexScreener, and GoPlus Security. Live on mainnet with USDC micropayments ($0.02/audit) via x402 protocol.
